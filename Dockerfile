...
RUN apt-get -qq update && DEBIAN_FRONTEND=noninteractive apt install -y curl sudo && \
    DEBIAN_FRONTEND=noninteractive apt -y install dos2unix git zip rsync mkisofs $(curl -fsSL https://raw.githubusercontent.com/P3TERX/openwrt-list/master/depends-ubuntu-2004) && \
